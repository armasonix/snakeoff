...
namespace render
{
    void drawUI(sf::RenderTarget& rt, UIParams u)
    {
        ...
        struct Pass { const char* name; bool enabled; std::function<void()> draw; };
        
            // helpers
            auto drawStage = [&]() 
            {
                if (!(u.stageTimer > 0.f)) return;
            ...
            auto drawScore = [&]() 
            {
                ...
                auto ch = [&](float p)->sf::Uint8 { float s = 0.5f + 0.5f * std::sin(phase + p); return (sf::Uint8)std::round(255.f * s); };
            ...
            auto drawBreaker = [&]() 
            {
                if (u.snake.breakerTimeLeft() <= 0.f) return;
                rt.draw(u.brBack);
                rt.draw(u.brFill);
                rt.draw(u.brText);
            };
            ...
            auto drawBanner = [&](sf::Text& t, float lifeT, float elapsed, float yOff) 
            {
                ...
                const std::string cur = t.getString();
                if (cur != prev) 
                {
            ...
            Pass uiPipe[] = 
            {
                { "stage",   u.stageTimer > 0.f, [&] { drawStage(); } },
                { "score",   true,                [&] { drawScore(); } },
                { "info",    (u.effects.spdRemain() > 0.f) || (u.effects.invRemain() > 0.f), [&] { drawInfo(); } },
                { "breaker", u.snake.breakerTimeLeft() > 0.f, [&] { drawBreaker(); } },
                { "banner_breaker", u.breakerBannerT > 0.f, [&] { drawBanner(u.breakerBanner, u.breakerBannerT, u.breakerBannerElapsed, -100.f); } },
                { "banner_turbo",   u.turboBannerT > 0.f, [&] { drawBanner(u.turboBanner,   u.turboBannerT,   u.turboBannerElapsed,   -60.f); } },
                { "banner_poison",  u.poisonBannerT > 0.f, [&] { drawBanner(u.poisonBanner,  u.poisonBannerT,  u.poisonBannerElapsed,  -20.f); } },
            };
        for (const auto& p : uiPipe) if (p.enabled) p.draw();
    }
}